...
class CombinedChartState extends ChartState<CombinedChart> {
    ...
    if (widget.controller.painter.highLightPerTapEnabled) {
      ...
      if (widget.controller.touchEventListener != null) {
        widget.controller.touchEventListener
            .onPerformHighlight(lastHighlighted, HighlightStatus.END);
      }
    } else {
  ...
  AxisEnabled get axisEnabled {
    ...
    return AxisEnabled(
        bottom: xAxisPosition == XAxisPosition.BOTH_SIDED ||
            xAxisPosition == XAxisPosition.BOTTOM,
        top: xAxisPosition == XAxisPosition.BOTH_SIDED ||
  ...
  bool _specialDragHighlight(Offset localPosition) {
    ...
    if (widget.controller.painter.highLightPerTapEnabled) {
      ...
      if (h != null) {
        h.highlightX = widget.controller
            .getValuesByTouchPoint(
                localPosition.dx, localPosition.dy, AxisDependency.LEFT)
            .x;
      ...
      if (widget.controller.touchEventListener != null) {
        widget.controller.touchEventListener
            .onPerformHighlight(lastHighlighted, HighlightStatus.START);
      }
    } else {
      lastHighlighted = null;
    }
  ...
  bool _canMove() {
    return (widget.controller.specialMoveEnabled &&
            ...
            lastHighlighted != null) ||
        (!widget.controller.specialMoveEnabled &&
            widget.controller.painter.highlightPerDragEnabled);
  }

  bool _specialMove(Offset localPosition) {
    if (_canMove()) {
      final highlighted = widget.controller.painter
          .getHighlightByTouchPoint(localPosition.dx, localPosition.dy);
      ...
      highlighted.highlightY = widget.controller
          .getValuesByTouchPoint(
              localPosition.dx, localPosition.dy, AxisDependency.LEFT)
          .y;
      ...
      if (highlighted?.equalTo(lastHighlighted) == false) {
        ...
        if (widget.controller.touchEventListener != null) {
          widget.controller.touchEventListener
              .onPerformHighlight(lastHighlighted, HighlightStatus.MOVE);
        }
      }
      return widget.controller.specialMoveEnabled;
    }
  ...
  void onMoveUpdate(OpsMoveUpdateDetails details) {
    print("onMoveUpdate");
    var scaled =
        tryScaleUsingAxis(details.localPoint.dx, details.localPoint.dy);
    if (scaled) {
      return;
    }
  ...
  void updatePainter() {
    if (widget.controller.painter.getData() != null &&
      ...
      widget.controller.painter.highlightValueForce(
          widget.controller.painter.highlightForced, false);
    }
  }
}
